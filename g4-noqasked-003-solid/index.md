...
Solid는 간단히 말해 **입체도형**입니다. 입체도형에는 직육면체, 원기둥, 구, 원뿔 등 다양한 모양이 있지요. Geant4에서도 Solid라는 개념 아래에 **하위 개념**으로서 G4Box(직육면체), GTubs(원기둥), G4Sphere(구), G4Cons(원뿔) 등을 만들어, 다양한 모양을 정의할 수 있게 하였습니다.
...
생성자가 뭔지 모르겠다면, 일단은 그냥 어떤 개념을 가진 대상을 만들 때 사용하는 함수라고 생각하세요. 여기서는 직육면체를 만드는 함수라고 생각하시면 됩니다.
...
- pSPhi: 시작 중심각. 케이크 조각처럼 부채꼴 기둥을 표현하고자 하는 경우에, 시작할 각도를 0~360도 사이에서 지정할 수 있음. 일반적인 원기둥의 경우 0을 적음
- pDPhi: 중심각의 변화량. 케이크 조각처럼 부채꼴 기둥을 표현하고자 하는 경우에, 중심각의 변화량을 0~360도 사이에서 지정할 수 있음. 일반적인 원기둥의 경우 360도(360. * deg)를 적음
